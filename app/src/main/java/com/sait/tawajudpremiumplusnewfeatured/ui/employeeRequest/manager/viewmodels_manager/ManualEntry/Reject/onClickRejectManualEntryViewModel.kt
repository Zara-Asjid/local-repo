...
class onClickRejectManualEntryViewModel (application: Application) : AndroidViewModel(application) {

    private val repository =
        onClickRejectManualEntryRepository(onClickRejectManualEntryDataSource())
    ...
    fun getonClickRejectManualEntryStatus(
        ...
        try {
            if (getApplication<TawajudApplication>().hasInternetConnection()) {
                val response =
                    repository.getonclickrejectManualEntryData(context, reject_manual_entry_request)
