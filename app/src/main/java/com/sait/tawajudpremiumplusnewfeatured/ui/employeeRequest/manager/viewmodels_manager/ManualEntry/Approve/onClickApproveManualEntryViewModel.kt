...
class onClickApproveManualEntryViewModel (application: Application) : AndroidViewModel(application) {

    private val repository = onClickApproveManualEntryRepository(onClickApproveManualEntryDataSource())
    ...
    fun getonClickApproveManualEntryStatus(context: Context, approve_manual_entry_request : onClickApproveManualEntryRequest) = viewModelScope.launch {
        ...
        try {
            if (getApplication<TawajudApplication>().hasInternetConnection()) {
                val response = repository.getonclickapproveManualEntryData(context, approve_manual_entry_request )
