package com.sait.tawajudpremiumplusnewfeatured.ui.contact
...
import com.sait.tawajudpremiumplusnewfeatured.ui.contact.viewmodels.ContactViewModel
...
class ContactFragment : BaseFragment(), OnClickListener {
    ...
    override fun onResume() {
        ...
        (activity as MainActivity).show_app_title(mContext!!.resources.getString(R.string.title_contact))
...
*/
    ...
    private fun addObserver() {
        viewModel.contactResponse.observe(viewLifecycleOwner) { event ->
                            ...
                            if (contactData != null) {
                                ...
                                if (UserShardPrefrences.getLanguage(mContext).equals("0")) {
                                   /* binding.txtCompanyName.text =
                                    ...
                                    binding.txtOrgEmail.text =
                                        UserShardPrefrences.getEmail(mContext!!).toString()
                                    binding.txtOrgPhone.text =
                                        UserShardPrefrences.getMobile(mContext!!).toString()
                                } else {
                                   /* binding.txtCompanyName.text =
                                        contactData.data.companyArabicName.toString()*/
                                    binding.txtCompanyName.textDirection=View.TEXT_DIRECTION_ANY_RTL
    ...
    private fun getContactDetails() {
        viewModel.getContactData(
            mContext!!,
            UserShardPrefrences.getUserInfo(context).fKEmployeeId.toString(),
