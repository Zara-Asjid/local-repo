...
import com.sait.tawajudpremiumplusnewfeatured.databinding.FragmentDashboardManagerBinding
...
class DashboardFragment_Manager : BaseFragment(), OnClickListener {
    private var _binding: FragmentDashboardManagerBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentDashboardManagerBinding.inflate(inflater, container, false)
//setUpChartAnyChart()
...
//        String previousMonthDate = DateTimeOp.getPreviousMonthDateTime("yyyy-MM-dd");
        ...
        binding.txtFromDateM.text = initialDateMonth
        binding.txtToDateM.text = finalDateMonth
...
*/
    ...
    private fun getDashboardDetails() {
        ...
        val dashboardRequest = DashboardRequest(
            0, 0, 0, binding.txtFromDateM.text.toString(),
            UserShardPrefrences.getLanguage(mContext)!!.toInt(),
            UserShardPrefrences.getUserInfo(context).fKEmployeeId, binding.txtToDateM.text.toString(),
            0,  0
        )
...
*/
    ...
    private fun setClickListeners(activity: MainActivity?) {
        ...
        binding.rlToDateM.setOnClickListener(this)
        binding.rlFromDateM.setOnClickListener(this)
 ...
 */
    ...
    override fun onClick(v: View?) {
        when (v?.id) {
            ...
            R.id.rl_to_date_m -> {
                datePicker(binding.txtToDateM.text.toString(),binding.txtToDateM)
            }


            R.id.rl_from_date_m -> {
                datePicker(binding.txtFromDateM.text.toString(),binding.txtFromDateM)

            }
            R.id.iv_search_icon ->{
                if(DateTime_Op.validateDates(binding.txtFromDateM.text.toString(),binding.txtToDateM.text.toString())){
                    callDashboardAPI()
                }
                else{
    ...
    private fun pickDate(day: Int, month: Int, year: Int, textView: TextView) {
        val datePickerFragment = DatePickerFragment.newInstance(year, month, day,textView.id,"DashboardFragment_Manager")
