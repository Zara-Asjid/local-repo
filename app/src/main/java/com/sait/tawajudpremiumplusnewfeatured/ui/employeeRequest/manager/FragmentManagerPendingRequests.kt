...
import com.sait.tawajudpremiumplusnewfeatured.databinding.FragmentPendingManagerRequestBinding
import com.sait.tawajudpremiumplusnewfeatured.items.SpinnerItem_BO_Type
import com.sait.tawajudpremiumplusnewfeatured.ui.employeeRequest.adapters.Pending_Adapter_Manager
...
import com.sait.tawajudpremiumplusnewfeatured.ui.employeeRequest.models.Leaves.pending_leaves_data
import com.sait.tawajudpremiumplusnewfeatured.ui.employeeRequest.models.ManualEntry.PendingManualEntryData
...
import com.sait.tawajudpremiumplusnewfeatured.ui.employeeRequest.models.Permission.PendingPermissionData
import com.sait.tawajudpremiumplusnewfeatured.ui.employeeRequest.models.Permission.PendingPermissonRequest

import com.sait.tawajudpremiumplusnewfeatured.ui.employeeRequest.manager.viewmodels_manager.Leaves.Approve.ApprovesViewModel_Manager
import com.sait.tawajudpremiumplusnewfeatured.ui.employeeRequest.manager.viewmodels_manager.Leaves.Reject.RejectViewModel_Manager
import com.sait.tawajudpremiumplusnewfeatured.ui.employeeRequest.viewmodels.Leaves.Pending.PendingLeavesViewModel
import com.sait.tawajudpremiumplusnewfeatured.ui.employeeRequest.manager.viewmodels_manager.ManualEntry.Pending.PendingManualEntryViewModel
import com.sait.tawajudpremiumplusnewfeatured.ui.employeeRequest.manager.viewmodels_manager.ManualEntry.Approve.onClickApproveManualEntryViewModel
import com.sait.tawajudpremiumplusnewfeatured.ui.employeeRequest.manager.viewmodels_manager.ManualEntry.Reject.onClickRejectManualEntryViewModel
...
import com.sait.tawajudpremiumplusnewfeatured.ui.employeeRequest.manager.viewmodels_manager.Permissions.Pending.PendingPermissionViewModel
import com.sait.tawajudpremiumplusnewfeatured.ui.employeeRequest.manager.viewmodels_manager.Permissions.Approve.onClickApprovePermViewModel
...
class FragmentManagerPendingRequests : BaseFragment(), View.OnClickListener, PendingLeaveApprovalListener_Manager,
    AdapterView.OnItemSelectedListener {
    private lateinit var binding: FragmentPendingManagerRequestBinding
    private lateinit var viewModel: PendingLeavesViewModel
    var spinnerItemBO_requsettype: SpinnerItem_BO_Type? = null
  //  var arrayList_type = ArrayList<String>()
    val arrayList_type = ArrayList<SpinnerItem_BO_Type>()
    var handler: Handler = Handler()

    private lateinit var pending_leave_viewModel: PendingLeavesViewModel
    private lateinit var approveViewModel: ApprovesViewModel_Manager
    private lateinit var rejectviewModel: RejectViewModel_Manager
    private lateinit var onclick_approv_permission_viewModel: onClickApprovePermViewModel
    private lateinit var onclick_approv_manual_entry_viewModel: onClickApproveManualEntryViewModel
    private lateinit var onclick_reject_manual_entry_viewModel: onClickRejectManualEntryViewModel

    private lateinit var pendingManualEntryviewModel: PendingManualEntryViewModel
    private lateinit var approvePermissionviewModel: ApprovePermissionViewModel

    private lateinit var pendingPermissionViewModel: PendingPermissionViewModel

    private var mContext: Context? = null
    private lateinit var viewModel_RequestType: RequestTypeViewModel

    private lateinit var pendingleaveslist: ArrayList<pending_leaves_data>
    private lateinit var pendingPermissionlist: ArrayList<PendingPermissionData>
    private lateinit var pendindManualEntrylist: ArrayList<PendingManualEntryData>
    var selectedPosition: Int = 0
    private var pendingAdapter: Pending_Adapter_Manager? = null
    ...
    override fun onCreateView(
        ...
        binding = FragmentPendingManagerRequestBinding.inflate(inflater, container, false)
        mContext = inflater.context
        sp_list = binding.spList
        viewModel_RequestType = ViewModelProvider(this)[RequestTypeViewModel::class.java]

        viewModel = ViewModelProvider(this)[PendingLeavesViewModel::class.java]
        approvePermissionviewModel =
            ViewModelProvider(this)[ApprovePermissionViewModel::class.java]
        approveViewModel = ViewModelProvider(this)[ApprovesViewModel_Manager::class.java]
        rejectviewModel = ViewModelProvider(this)[RejectViewModel_Manager::class.java]
        onclick_approv_permission_viewModel= ViewModelProvider(this)[onClickApprovePermViewModel::class.java]
        onclick_approv_manual_entry_viewModel=ViewModelProvider(this)[onClickApproveManualEntryViewModel::class.java]
        onclick_reject_manual_entry_viewModel=ViewModelProvider(this)[onClickRejectManualEntryViewModel::class.java]

        pendingManualEntryviewModel = ViewModelProvider(this)[PendingManualEntryViewModel::class.java]


        pendingPermissionViewModel = ViewModelProvider(this)[PendingPermissionViewModel::class.java]
...
*/
    ...
    private fun setAdapter() {
       // if (pendingAdapter == null || binding.recyclerView.adapter == null) {
            ...
            pendingAdapter = Pending_Adapter_Manager(
                pendingleaveslist,
    ...
    private fun getallPendingManualEntry() {
        val pendingmanualEntryRequest = PendingManualEntryRequest(
            ...
            UserShardPrefrences.getLanguage(mContext)!!.toInt(),
            0,
            binding.txtManagerFromDatePendReq.text.toString(),
            binding.txtManagerToDatePendReq.text.toString()
        )
...
if(pendingManualEntryResponse.message!!.contains(mContext!!.resources.getString(R.string.no_record_found_txt))){
    ...
    private fun getallpendingPermissions() {
        ...
        pendingPermissionViewModel.getPendingPermissionStatus(
            mContext!!,
    ...
    private fun addObserverForPendingLeaves() {
        viewModel.pending_leaves_response.observe(viewLifecycleOwner) { event ->
            event.getContentIfNotHandled()?.let { response ->
                when (response) {
                    is Resource.Success -> {
                      /*  binding.rvSwipeRefresh.isRefreshing = false
...
//        String previousMonthDate = DateTimeOp.getPreviousMonthDateTime("yyyy-MM-dd");
        ...
        binding.txtManagerFromDatePendReq.setText(initialDateMonth)
        binding.txtManagerToDatePendReq.setText(finalDateMonth)

    }

    private fun setClickListeners(activity: MainActivity?) {
        activity?.binding!!.layout.imgBack.setOnClickListener(this)
        binding.rlManagerToDatePendReq.setOnClickListener(this)
        binding.rlManagerFromDatePendReq.setOnClickListener(this)
    ...
    override fun onClick(v: View?) {
        when (v?.id) {
            ...
            R.id.rl_manager_to_date_pend_req -> {
                datePicker(binding.txtManagerToDatePendReq.text.toString(), binding.txtManagerToDatePendReq)
            }

            R.id.rl_manager_from_date_pend_req -> {
                datePicker(binding.txtManagerFromDatePendReq.text.toString(), binding.txtManagerFromDatePendReq)

            }
            R.id.iv_search_icon -> {


                if(DateTime_Op.validateDates(binding.txtManagerFromDatePendReq.text.toString(),binding.txtManagerToDatePendReq.text.toString())){
                    when{
                        (selectedPosition==0)->{
                            callAllPendingPermissionsAPI()
...
*/
    ...
    override fun onApproveItemClick(position: Int, type: String) {
        ...
        title.text = if (status_id == 2) getString(R.string.approve) else getString(R.string.reject)
        ...
        submit.setOnClickListener {
            if (EnumUtils.isNetworkConnected(context)) {
                        callRejectOrAccept(type, type, position, 2)
            } else {
      ...
      //  cancel.setTextColor(colorPrimary)
        ...
        submit.setOnClickListener {
            ...
            if (EnumUtils.isNetworkConnected(context)) {
                when (type) {
                    "L" -> {
                    callRejectOrAccept(type,remarks,position,4,)
                    }
                    "M" -> {
                    callRejectOrAccept(type,remarks,  position, 4)
                    }
    ...
    private fun callRejectOrAccept(
        ...
        if (nextApprovalStatus==2 && type.equals("L")){
            CallforSaveApprovedLeaveAPI(position,type,remarks)
        }
        if (nextApprovalStatus==4 && type.equals("L")){
            CallforRejectbyManagerAPI(position,  remarks ,nextApprovalStatus,type)
        }
        if (nextApprovalStatus==2 && type.equals("P")){
            CallforSaveApprovedPermissionAPI(position,2,remarks)
        }
        if (nextApprovalStatus==4 && type.equals("P")){
            CallforRejectbyManagerAPI(position,  remarks,nextApprovalStatus,type)
        }
        if (nextApprovalStatus==2 && type.equals("M")){
            CallForsaveManualEntryRequestAPI(position,2)
        }
        if (nextApprovalStatus==4 && type.equals("M")){
...
*/
    ...
    private fun CallforSaveApprovedPermissionAPI(
        position: Int,
        nextApprovalStatus: Int,
        remarks: String
    ) {
             addObserverForSaveApprovePermissionRequest()
            ApprovePermissionRequestByManager(nextApprovalStatus,position,remarks)
    }

    private fun ApprovePermissionRequestByManager(
        nextApprovalStatus: Int,
        position: Int,
        remarks: String
    ) {
        val approvePermissionData = onClickApproveRequest(
            ...
            UserShardPrefrences.getLanguage(mContext)!!.toInt(),
            nextApprovalStatus,
            pendingPermissionlist[position].fK_PermId,
            remarks
        )


        onclick_approv_permission_viewModel.getonClickApprovePermissionStatus(
            mContext!!,
    ...
    private fun addObserverForSaveApprovePermissionRequest() {
        onclick_approv_permission_viewModel.onclick_approve_permission_response.observe(viewLifecycleOwner) { event ->
            event.getContentIfNotHandled()?.let { response ->
                when (response) {
                    is Resource.Success -> {
                      /*  binding.rvSwipeRefresh.isRefreshing = false
...
*/
    ...
    private fun CallforRejectbyManagerAPI(
        position: Int,
        remarks: String,
        nextApprovalStatus: Int,
    type: String
        ...
        getRejectLeaveStatus(type,position, remarks, nextApprovalStatus)
    }

    @SuppressLint("SuspiciousIndentation")
    private fun getRejectLeaveStatus(type: String, position: Int, remarks: String, nextApprovalStatus: Int) {
        if (type.equals("L")){
        val rejectLeaveData = RejectLeaveByManagerRequest(
            ...
            rejectviewModel.getRejectLeaveStatus(
                mContext!!,
                rejectLeaveData
            )
    ...
    private fun CallforSaveApprovedLeaveAPI(
        ...
        getonClickApproveRequest(position,type,remarks)
    }
    ...
    private fun getonClickApproveRequest(
        ...
        val approveLeavesRequest = Approved_ByManagerRequest(
        ...
        approveViewModel.getApproveLeaves(
            mContext!!,
    ...
    private fun addObserverForApproveRequest() {
        approveViewModel.approve_leaves_response.observe(viewLifecycleOwner) { event ->
            event.getContentIfNotHandled()?.let { response ->
                when (response) {
                    is Resource.Success -> {
                      /*  binding.rvSwipeRefresh.isRefreshing = false
                        ...
                        response.data?.let { approveLeaveResponse ->
                            ...
                            if (approveLeaveResponse != null) {
                                ...
                                if (approveLeaveResponse.message!!.contains(mContext!!.resources.getString(R.string.response_error_txt))) {
                                    binding.txtNoRecord.visibility = View.VISIBLE
                                  //  binding.rvSwipeRefresh.visibility = View.GONE
                                    SnackBarUtil.showSnackbar_Drawable_backroundcolor(context,approveLeaveResponse.data.error,R.drawable.caution,resources.getColor(R.color.red))
                                ...
                                //
                                    ...
                                    SnackBarUtil.showSnackbar_Drawable_backroundcolor(context,approveLeaveResponse.data.success,R.drawable.app_icon,colorPrimary,
                                        SnackBarUtil.OnClickListenerNew {
