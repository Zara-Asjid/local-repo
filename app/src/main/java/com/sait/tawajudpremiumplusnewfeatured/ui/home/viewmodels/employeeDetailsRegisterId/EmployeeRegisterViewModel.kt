...
class EmployeeRegisterViewModel(application: Application) : AndroidViewModel(application) {

    private val repository = EmployeeRegisterRepository(EmployeeRegisterDataSource())
    ...
    fun getEmployeeRegisterData(context: Context, employeeRequest: EmpDetailsRequest) = viewModelScope.launch {

        _employeeResponse.postValue(Event(Resource.Loading()))

        try {
            if (getApplication<TawajudApplication>().hasInternetConnection()) {
                val response = repository.getEmployeeRegisterData(context,employeeRequest )
