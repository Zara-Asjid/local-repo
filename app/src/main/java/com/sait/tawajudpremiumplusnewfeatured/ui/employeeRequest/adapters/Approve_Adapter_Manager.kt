...
import com.sait.tawajudpremiumplusnewfeatured.ui.employeeRequest.manager.FragmentManagerApproveRequest
...
class Approve_Adapter_Manager(
    ...
    private val Listener: FragmentManagerApproveRequest,
    private var mContext: Context
) : RecyclerSwipeAdapter<Approve_Adapter_Manager.SimpleViewHolder>() {
...
/*
        if (approveLeaveRequestList!!.size != 0) {
            ...
            viewHolder.tv_status.text = mContext!!.getString(R.string.approved)
            viewHolder.tvAction.text = items.employeeNo+" "+items.employeeName
            viewHolder.tv_permission.text = items.leaveName

            viewHolder.tv_date.text = FormatingDate(items.requestDate)
            viewHolder.tv_to_time!!.text = FormatingDate(items.toDate)
            viewHolder.tv_from_time.text = FormatingDate(items.fromDate)
            viewHolder.request_date!!.text = "Requested Date:"
            viewHolder.from_date!!.text = "From Date:"
            viewHolder.to_date.text = "To Date:"
            viewHolder.permission_leave_tv.text = mContext!!.resources.getString(R.string.emp_leave)
...
*/
        ...
        if (approvePermissionRequestList!!.size != 0) {
            ...
            val items: ApprovePermissionData = approvePermissionRequestList!![position]
                    ...
                    viewHolder.tv_action.text = if(UserShardPrefrences.getLanguage(mContext)!!.equals("0"))  items.employeeNo+" "+items.employeeName else  items.employeeNo+" "+items.employeeArabicName
                    viewHolder.txt_permission.text =  if(UserShardPrefrences.getLanguage(mContext)!!.equals("0")) items.permName else items.permArabicName
                    viewHolder.txt_status.text = if(UserShardPrefrences.getLanguage(mContext)!!.equals("0")) items.statusName else items.statusNameArabic
                        ...
                        viewHolder.txt_req_date.text = LocaleHelper.arabicToEnglish(FormatingDate(items.permDate).toString())
                    ...
                    // viewHolder.tv_permission.text=items.leaveName
                    // viewHolder.tv_time.text = FormatingDate(items.requestDate)
                    // viewHolder.tv_from_time.text = FormatingTime(items.fromDate)
                    // viewHolder.tv_date.text = FormatingDate(items.toDate)
