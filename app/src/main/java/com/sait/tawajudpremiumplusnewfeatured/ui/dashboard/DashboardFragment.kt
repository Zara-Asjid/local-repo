...
class DashboardFragment : BaseFragment() , View.OnClickListener {
    ...
    private val checkInternetRunnable = object : Runnable {
        ...
        override fun run() {
            val application = context?.applicationContext as? TawajudApplication
            if (application != null && isAdded) {
    ...
    // Clean up the handler when the Fragment view is destroyed
    ...
    private fun setAdapters() {
        ...
        adapter!!.addFragment(DashboardFragment_self(), mContext!!.resources.getString(R.string.self_dashboard_txt))
        //adapter!!.addFragment(Reports_ManagerFragment(), "Manager Reports")

        if(UserShardPrefrences.getisManager(mContext)){
            adapter!!.addFragment(DashboardFragment_Manager(), mContext!!.resources.getString(R.string.manager_dashboard_txt))
        }

//UserShardPrefrences.getisHRAdmin(mContext)||
        if(UserShardPrefrences.getisAdmin(mContext)){
            adapter!!.addFragment(DashboardFragment_Admin(), mContext!!.resources.getString(R.string.admin_dashboard_txt))
        }
