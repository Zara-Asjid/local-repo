...
class ApproveLeaveViewModel (application: Application) : AndroidViewModel(application) {



    private val repository = ApproveLeavesRepository(ApproveLeaveDataSource())

    private val _approveLeavesResponse = MutableLiveData<Event<Resource<Approve_LeavesResponse>>>()
    val approver_leaves_response: LiveData<Event<Resource<Approve_LeavesResponse>>> = _approveLeavesResponse
    ...
    fun getAllApproveLeaves(context: Context, approve_leave_Request : Approve_LeavesRequest) = viewModelScope.launch {
        ...
        try {
            if (getApplication<TawajudApplication>().hasInternetConnection()) {
                val response = repository.getApproveLeavesData(context, approve_leave_Request )
